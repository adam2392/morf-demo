...
def _load_turbo():
    """Load turbo colormap into matplotlib registry."""
    mpl_data = _RGBToPyCmap(turbo_colormap_data)
    cmap = LinearSegmentedColormap(
        name="turbo", segmentdata=mpl_data, N=turbo_colormap_data.shape[0]
    )
    plt.register_cmap(cmap=cmap)

    mpl_data_r = _RGBToPyCmap(turbo_colormap_data[::-1, :])
    cmap = LinearSegmentedColormap(
        name="turbo)r", segmentdata=mpl_data_r, N=turbo_colormap_data.shape[0]
    )
    plt.register_cmap(cmap=cmap)
